...
export interface SubqlCustomDatasource<K extends string = string, T extends SubqlNetworkFilter = SubqlNetworkFilter, M extends SubqlMapping = SubqlMapping<SubqlCustomHandler>, O = any> extends ISubqlDatasource<M, T> {
    kind: K;
    assets: Map<string, CustomDataSourceAsset>;
    processor: Processor<O>;
}
export interface HandlerInputTransformer<T extends SubqlHandlerKind, U, DS extends SubqlCustomDatasource = SubqlCustomDatasource> {
    (original: RuntimeHandlerInputMap[T], ds: DS, api: ApiPromise, assets: Record<string, string>): Promise<U>;
}
export interface SubqlDatasourceProcessor<K extends string, F extends SubqlNetworkFilter, DS extends SubqlCustomDatasource<K, F> = SubqlCustomDatasource<K, F>> {
    kind: K;
    validate(ds: DS, assets: Record<string, string>): void;
    dsFilterProcessor(ds: DS, api: ApiPromise): boolean;
    handlerProcessors: {
        [kind: string]: SecondLayerHandlerProcessor<SubqlHandlerKind, unknown, unknown, DS>;
    };
}
export interface SecondLayerHandlerProcessor<K extends SubqlHandlerKind, F, E, DS extends SubqlCustomDatasource = SubqlCustomDatasource> {
    baseHandlerKind: K;
